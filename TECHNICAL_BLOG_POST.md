...
* **Dramatically Lowers Trial Costs**: Brings clinical trial monitoring costs down to under $0.05 per patient session on AWS Serverless.
...
3. **10s Incident Ring Buffer**: Retains 150 pre-trigger frames in local RAM. When an oscillation anomaly (2.5 to 6.0 Hz) occurs, it captures 150 post-trigger frames, stitches a **10-second WebM clip** (in under 80ms), and uploads it to an encrypted Amazon S3 vault.
...
Includes an interactive Color-Word Stroop conflict challenge (e.g., the word "BLUE" printed in red font) with 1.5-second rapid decision windows to evaluate vagal withdrawal under controlled cognitive load.
...
* **Pre-Trigger Buffer**: Keeps 150 frames (5.0 seconds at 30 FPS) in volatile JavaScript RAM.
* **In-Browser WebM Multiplexer (`webm_encoder.js`)**: A custom, zero-dependency 8KB EBML/VP8 multiplexer that packages all 300 frames (10.0 seconds) into a seekable WebM blob in under 80 milliseconds (replacing heavy 25MB WebAssembly encoders).
* **6-Second Cooldown**: Locks out duplicate triggers for 180 frames after clip generation.

### D. Autonomic Stress Drop Mathematics
Connecting via Web Bluetooth GATT (Service 0x180D / Characteristic 0x2A37), the system parses beat-to-beat R-R intervals to calculate **RMSSD (Root Mean Square of Successive Differences)**.

During an oscillation flare, it computes the **Autonomic Stress Drop Percentage**:
`Stress Drop % = ((Baseline RMSSD - Incident RMSSD) / Baseline RMSSD) * 100`

A drop of 25% or higher confirms a **stress-triggered neurological exacerbation** rather than an incidental head turn.
...
* **1-Click Triage & Dynamic Graphing**: Review 10-second video clips with slow-motion HUD overlays (0.5x / 1.0x) and click `✓ Verify TP` or `✕ Dismiss FP`, instantly updating DynamoDB and plotting verified points to trend graphs.
